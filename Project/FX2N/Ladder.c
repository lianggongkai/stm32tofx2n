...
{switch(process_addr)
	{ 
		case 0xF8:   ANB()               ;	 break;    //??? ANB
		case 0xF9:   ORB()               ;	 break;    //??? ORB
		case 0xFA:   MPS()               ;  break;    //??   MPS
		case 0xFB:   MRD()               ;  break;	   //??   MRD
		case 0xFD:   INV()               ;  break;    //??   INV
		case 0xFF: 					    break;	  //ADD
		default:  ;                       break;	
	}
}
...
static void DEC(void)	   	 //ADD TKZSLD	//???? ?1??
{
	if((process_value&0X01)==0X01)
	{
		sub_add1=cos_value();
...
{ switch(*p_prog) 
	{case 0x0002: p_prog++,extend_OUT_M();            break;  
		...
		case 0x0040: p_prog++,INC();                     break;  //ADD BY TKZSLD 
		case 0x0042: p_prog++,DEC();                     break;  //ADD BY TKZSLD
		...
		case 0x1040: p_prog++,extend_INCP();              break;  //16?????????1??  tkzsld
		case 0x1042: p_prog++,extend_DECP();              break;  //16?????????1??	tkzsld
...
{ 
	...
	if(1/*RUN_STOP==0*/)	  //MY PCB== 0	 AIR=1
	{
