/********************************************************/
// CPU??:STM32F103--RAM?????64K	Flash?????128K
// ?????STM32F103RDT6????
// ????:20150903
// editor by DaShi
/********************************************************/
...
//flash to save program generated from GXwork 
...
const char plc_type[2]={0XF6,0X5E};	  	  //PLC??  FX2N
...
{  float temp;
	...
	temp=(float)(72*1000000)/(19200*16);
	mantissa=temp;				 
...
/*	prog_write_buffer[] : The ram data is receave from the usart with PC
 *	The function is write the ram data to flash
 */
...
/*
 *	p_x[] : is the flash address 
 *	The func is read the ladder program to ram
 */
...
//PLC usart rx process data
void RX_Process(void)
{ 
	//0x7f & rcv data mean the PLC usart data is 7 bits;
	rx_data[0]=0x7f&USART_ReceiveData(USART1);
	if(rx_data[0]==0X05)//PC query plc is existing		
		rx_count=0,tx_data[1]=0x06,tx_count=1,rx_end=5; 

	if(rx_data[0]==0X02)//STX recv then continue receave the remain data
		rx_count=0x01;
	...
	if(rx_count>0)		  
	{
		...
		if((rx_count>3)&&(rx_data[rx_count-2]==0x03))//ETX recv mean PLC need CRC check (SUM CRC)	 
		{ 
			rx_data_sum();//sum crc check RX data frame is valid,if valid set rx_data[599] 1	
			if(rx_data[599]==1)//CRC OK		
				rx_end=1;//This Mean Recv the PC CMD
			else //CRC failed
				tx_data[1]=0x15,tx_count=1,rx_end=5;	  
		}
	}	   
...
//Write power-down saving data to disk/flash
...
//Read power-down saving data from disk/flash
...
//Write all the power-down saveing data to disk/flash
...
//Read all the power-down saving data from disk/flash
void recover_data(void)
{ u16 temp=0,backup_addr;
	read_data(10);
